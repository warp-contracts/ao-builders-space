...
jQuery( document ).ready(
	function() {
		...
		gamic_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="gamic_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.gamic_post_editor' );
						if ( link.length > 0 ) {
							gamic_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.gamic_post_editor', function(e) {
						gamic_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('gamic_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function gamic_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
