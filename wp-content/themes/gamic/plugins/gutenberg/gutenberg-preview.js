...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			gamic_create_observer( 'gamic-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				gamic_gutenberg_editor_init();
			} );
		} else {
		 	gamic_gutenberg_editor_init();
		}
	...
	function gamic_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'gamic_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'gamic_inited' ) ) {
	...
	// Init on page load
	function gamic_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = gamic_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + GAMIC_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + GAMIC_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in GAMIC_STORAGE['override_classes'] ) {
				$obj.addClass( GAMIC_STORAGE['override_classes'][i].replace( '%s', GAMIC_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( gamic_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && GAMIC_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'gamic_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			gamic_remove_observer( 'gamic-check-editor-styles-wrapper' );
			gamic_create_observer( 'gamic-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'gamic_inited' );
	}
} );
